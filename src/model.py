...
def log_metadata(cfg, gs, X_train, y_train, X_test, y_test):
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as run:
        ...
        df_train_dataset = mlflow.data.pandas_dataset.from_pandas(df=df_train,
                                                                  targets=cfg.data.target_cols[0])  # type: ignore
        ...
        for index, result in cv_results.iterrows():
            ...
            with mlflow.start_run(run_name=child_run_name, experiment_id=experiment_id,
                                  nested=True):  # , tags=best_metrics_dict):
...
def train(X_train, y_train, cfg):
    ...
    # Create a pipeline that first preprocesses the data and then fits the estimator
    ...
    # param_grid = {k: [ast.literal_eval(val) if isinstance(val, str) else val for val in v]
    ...
    scoring = list(cfg.model.metrics.values())  # e.g., ['balanced_accuracy', 'f1_weighted', 'precision', 'recall', 'roc_auc']
